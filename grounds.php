...
<body>
    ...
    locationButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            filterVenues(location);
        });
    });
    
    function filterVenues(location) {
        const venues = document.querySelectorAll('.academy');
        
        venues.forEach(venue => {
            const venueLocation = venue.querySelector('.turf-details:nth-child(2)').textContent.trim().toLowerCase();
            ...
            if (location === 'all') {
                ...
                if (locationMap[location] && venueLocation.includes(locationMap[location])) {
                    venue.style.display = 'block';
                } else if (location === 'other' && 
                          !venueLocation.includes('panjim') && 
                          !venueLocation.includes('margao') && 
                          !venueLocation.includes('mapusa')) {
                    venue.style.display = 'block';
                } else {
    ...
    searchInput.addEventListener('input', function() {
        ...
        venues.forEach(venue => {
            const venueName = venue.querySelector('.turf-details:nth-child(1)').textContent.toLowerCase();
            const venueLocation = venue.querySelector('.turf-details:nth-child(2)').textContent.toLowerCase();
